    ...
    public class WeaponComponent : TorqueComponent
    {
        ...
        public void FireAtTarget(Vector2 target)
        {
            if (projectileTemplate.IsTemplate)
            {
                ...
                //theta = -MathHelper.ToDegrees(theta);
                projectile.Position = SceneObject.Position;
                Vector2 direction = T2DVectorUtil.VectorFromAngle(theta);
                //direction.Normalize();
                projectile.Physics.VelocityY = direction.Y * shotSpeed;
                projectile.Physics.VelocityX =  -(2*distance) / (2);
